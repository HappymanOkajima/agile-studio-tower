...
iOS Safariは他のブラウザと異なるタッチイベント処理を行う。Kaboomの抽象化レイヤーがiOSのタッチイベントを正しくキャプチャできない場合がある。
...
if (canvas) {
  canvas.addEventListener('touchstart', (e: TouchEvent) => {
    ...
    const scaleX = 400 / rect.width;  // ゲーム幅 / 実際のCanvas幅
    const scaleY = 800 / rect.height; // ゲーム高さ / 実際のCanvas高さ
    const x = (touch.clientX - rect.left) * scaleX;
    const y = (touch.clientY - rect.top) * scaleY;

    // ボタン領域チェック（中心座標とサイズから判定）
    const btnX = 200, btnY = 600;
    const halfWidth = 90, halfHeight = 22;
    if (x >= btnX - halfWidth && x <= btnX + halfWidth &&
...
1. **座標変換が必須**: レスポンシブデザインでCanvasサイズが変わるため、タッチ座標をゲーム座標系に変換する
2. **`passive: false`**: `e.preventDefault()`を使うために必要
3. **ボタンごとに領域判定**: 複数ボタンがある場合は座標で判別
4. **重複実行防止**: 状態フラグ（例: `retried`）で二重実行を防ぐ

### 影響を受ける画面

- **タイトル画面**: STARTボタン
- **ゲーム画面**: ブロック落下（画面全体タップ）
- **リザルト画面**: RETRYボタン、コンテンツリンクボタン

### 検証方法

1. iPhone実機またはXcodeシミュレーターで確認
2. Chrome DevToolsのデバイスエミュレーションは不十分（実機と挙動が異なる）
